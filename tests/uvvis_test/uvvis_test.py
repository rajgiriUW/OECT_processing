...
class TestUVVis:
    ...
    def test_single_time_spectra(self, read_files, data_load, dataset, specs, expected_values_folder):
        try:
            ...
            time_spectra = data_load[dataset]._single_time_spectra(spectra_path=read_files[specs][0],
                                                                   smooth=uvvis_inputs.single_time_smooth,
                                                                   digits=uvvis_inputs.single_time_digits)
    ...
    @pytest.mark.parametrize("dataset, specs, expected_values_folder", \
                             [("doping", "specs",
                               os.path.join(expected_values_folder, "expected_data_spectra_vs_time.npy")), \
                              ("dedoping", "dedopespecs",
                               os.path.join(expected_values_folder, "expected_dedata_spectra_vs_time.npy"))])
    def test_time_dep_spectra(self, read_files, data_load, dataset, specs, expected_values_folder):
        try:
            data_load[dataset].time_dep_spectra(specfiles=read_files[specs], smooth=uvvis_inputs.time_dep_smooth,
                                                round_wl=uvvis_inputs.time_dep_round_wl,
    ...
    def time_dep_spectra(self, read_files, data_load):
        ...
        data_load["dedoping"].time_dep_spectra(read_files["dedopespecs"], smooth=uvvis_inputs.time_dep_smooth,
                                               round_wl=uvvis_inputs.time_dep_round_wl,
                                               droptimes=uvvis_inputs.droptimes)
    ...
    @pytest.mark.parametrize("dataset, specs, data_label",
                             [("doping", "specs", "data"), ("dedoping", "specs", "dedata")])
    def test_single_wl_time(self, read_files, data_load, dataset, specs, data_label, time_dep_spectra):
        try:
            ...
            data_load[dataset].single_wl_time(wavelength=uvvis_inputs.single_wl_wavelength,
                                              potential=uvvis_inputs.single_wl_potential,
                                              smooth=uvvis_inputs.single_wl_smooth)
            expected_time_spectra = np.load(
                os.path.join(expected_values_folder, "expected_%s_time_spectra.npy" % data_label))
            expected_time_spectra_norm = np.load(
                os.path.join(expected_values_folder, "expected_%s_time_spectra_norm.npy" % data_label))
            expected_time_spectra_sm = np.load(
                os.path.join(expected_values_folder, "expected_%s_time_spectra_sm.npy" % data_label))
            expected_time_spectra_norm_sm = np.load(
                os.path.join(expected_values_folder, "expected_%s_time_spectra_norm_sm.npy" % data_label))
    ...
    @pytest.mark.dependency(depends=["read_files", "data_load"])
    @pytest.mark.parametrize("dataset, steps, data_label",
                             [("doping", "steps", "data"), ("dedoping", "dedopesteps", "dedata")])
    def test_current_extraction(self, data_load, read_files, dataset, steps, data_label):
        try:
            ...
            expected_current = pd.read_pickle(
                os.path.join(expected_values_folder, "expected_%s_current.pkl" % data_label))
            expected_charge = pd.read_pickle(
                os.path.join(expected_values_folder, "expected_%s_charge.pkl" % data_label))
    ...
    def test_spec_echem_voltage(self, read_files, data_load, dataset, specs, data_label, time_dep_spectra):
        try:
            data_load[dataset].spectra_vs_time = time_dep_spectra[dataset]
            data_load[dataset].spec_echem_voltage(time=uvvis_inputs.spec_echem_time,
                                                  smooth=uvvis_inputs.spec_echem_smooth,
                                                  digits=uvvis_inputs.spec_echem_smooth)
            spectra = data_load[dataset].spectra
            spectra_sm = data_load[dataset].spectra_sm
            expected_spectra = pd.read_pickle(
                os.path.join(expected_values_folder, "expected_%s_spectra.pkl" % data_label))
            expected_spectra_sm = pd.read_pickle(
                os.path.join(expected_values_folder, "expected_%s_spectra_sm.pkl" % data_label))
    ...
    def test_abs_vs_voltage(self, read_files, data_load, time_dep_spectra, dataset, data_label):
        try:
            ...
            expected_vt = pd.read_pickle(
                os.path.join(expected_values_folder, "expected_%s_abs_vs_volt.pkl" % data_label))
    ...
    @pytest.mark.parametrize("dataset, fitfunc, data_label", [("doping", "exp", "data"), ("dedoping", "exp", "dedata"), \
                                                              ("doping", "biexp", "data"),
                                                              ("dedoping", "biexp", "dedata"),
                                                              ("doping", "stretched", "data"),
                                                              ("dedoping", "stretched", "dedata"), ])
    def test_banded_fits(self, data_load, time_dep_spectra, dataset, fitfunc, data_label):
        try:
            ...
            expected_fits = np.load(
                os.path.join(expected_values_folder, "expected_%s_%s_banded_fits.npy" % (data_label, fitfunc)))
            assert True
        except:
            assert False
